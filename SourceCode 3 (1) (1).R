...
path = read.csv("Cleaned_Dataset (1).csv", header = TRUE, stringsAsFactors = F)

#path = read.csv("Lifestyle_and_Health_Risk_Prediction_Synthetic_Dataset.csv", header = TRUE, stringsAsFactors = F)
...
# path$exercise <- as.factor(path$exercise)
# path$sugar_intake <- as.factor(path$sugar_intake)
...
logit.pred <- predict(logit.reg, test.path, type = "response");logit.pred
